...
function toApiError(error) {
    ...
    if (error instanceof offline_queue_js_1.OfflineQueuedError) {
        return {
...
function sanitizeHeadersMap(headers, extraSensitive = []) {
    ...
    const blocked = new Set([
        ...types_js_1.DEFAULT_SENSITIVE_HEADERS.map((h) => h.toLowerCase()),
...
function createRestClient(config) {
    ...
    const responseCache = (_b = (_a = config.cache) === null || _a === void 0 ? void 0 : _a.store) !== null && _b !== void 0 ? _b : new cache_js_1.TtlCache(1000);
    // --- Rate limiter ---
    const rateLimiter = config.rateLimit
        ? new rate_limiter_js_1.RateLimiter(config.rateLimit)
        : null;
    // --- Circuit breaker ---
    const circuitBreaker = config.circuitBreaker
        ? new circuit_breaker_js_1.CircuitBreaker(config.circuitBreaker)
        : null;
    // --- Offline queue ---
    ...
    const offlineQueue = ((_c = config.offlineQueue) === null || _c === void 0 ? void 0 : _c.enabled)
        ? new offline_queue_js_1.OfflineQueue(config.offlineQueue, (queued) => _executeRequest(queued.url, {
    ...
    async function _executeRequest(command, req, _retried = false) {
        ...
        if (circuitBreaker && !(await circuitBreaker.canExecute())) {
            const apiError = toApiError(new circuit_breaker_js_1.CircuitOpenError());
            ...
            throw new circuit_breaker_js_1.CircuitOpenError();
        }
        ...
        try {
            ...
            const errorHeaders = axios_1.default.isAxiosError(error)
    ...
    async function request(command, req, _retried = false) {
        ...
        if (offlineQueue &&
            ...
            const queuedError = new offline_queue_js_1.OfflineQueuedError(queued.id, methodUpper, command);
