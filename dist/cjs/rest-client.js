...
function createRestClient(config) {
    ...
    const circuitBreaker = config.circuitBreaker
        ? new circuit_breaker_1.CircuitBreaker(config.circuitBreaker)
        : null;
    ...
    async function _executeRequest(command, req, _retried = false) {
        ...
        if (circuitBreaker && !circuitBreaker.canExecute()) {
            const apiError = toApiError(new circuit_breaker_1.CircuitOpenError());
            ...
            throw new circuit_breaker_1.CircuitOpenError();
        }
